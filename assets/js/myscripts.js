...
// function showMeme(){
  showMeme=()=>{
const geturl=getRandomData('memes');
    const meme = document.querySelector('.meme-content');
const newelement=document.createElement('img');
newelement.setAttribute('src',geturl);
clearall();
meme.appendChild(newelement);

}
function showJoke(){
  console.log('joke');
const gettext=getRandomData('jokes');
const newelement=document.createElement('p');
newelement.textContent=gettext; 
clearall();
document.querySelector('#joke-content').appendChild(newelement);
}
function showQuote(){
...
quotee.textContent=getquote.quote;
authorr.textContent="-"+getquote.author;
clearall();
document.querySelector('.quote-content').appendChild(quotee);
document.querySelector('.quote-content').appendChild(authorr);
console.log(getquote.author);
}
function showRiddle(){
const getriddle=getRandomData('riddles');
// const{question,answer}=getriddle; //another way to take value out of the object
const newriddle=document.createElement('p');
const ans=document.createElement('p');
ans.setAttribute('id','answer-content');
ans.textContent="-"+getriddle.answer;
newriddle.textContent=getriddle.question;
clearall();
ans.hidden=true;
document.querySelector('.riddle-content').appendChild(newriddle);
document.querySelector('.riddle-content').appendChild(ans);
...
const riddleans=document.querySelector('#answer-content');
...
function getRandomData(type) {
  //data[type ] return value inside the array
  console.log(rn(data[type].length));// return number
    return data[type][rn(data[type].length)];
  }
